    ...
    public class InlineMarkupConditional : IOperationProvider
    {
        ...
        public class Impl : IOperation
        {
            ...
            public int HandleMatch(IProcessorState processor, int bufferLength, ref int currentBufferPosition, int token, Stream target)
            {
                ...
                IProcessorState localState = new ProcessorState(new MemoryStream(condition), new MemoryStream(), conditionBytes.Count, int.MaxValue, adjustedConfig, new IOperationProvider[0], null);
                int pos = 0;
