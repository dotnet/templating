        ...
        private class Impl : IOperation
        {
            ...
            public int HandleMatch(IProcessorState processor, int bufferLength, ref int currentBufferPosition, int token, StreamWriteAction targetWriteAction)
            {
                ...
                if (_depth < 0)
                {
                    ...
                    processor.Config.Logger.LogInformation($"Balanced nesting depth < 0. CurrentBufferPosition = {currentBufferPosition}");
                }

                if (_depth == 0 && token == PseudoEndTokenIndex)
                {
                    targetWriteAction(_realEndToken.Value, _realEndToken.Start, _realEndToken.Length);
