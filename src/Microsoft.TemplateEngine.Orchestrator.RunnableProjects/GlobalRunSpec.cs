...
namespace Microsoft.TemplateEngine.Orchestrator.RunnableProjects
{
    internal class GlobalRunSpec : IGlobalRunSpec
    {
            ...
            IRunnableProjectConfig configuration)
        {
            ...
            Operations = ResolveOperations(configuration.OperationConfig, templateRoot, variables);
            List<KeyValuePair<IPathMatcher, IRunSpec>> specials = new();

            if (configuration.SpecialOperationConfig != null)
            {
                foreach ((string glob, IGlobalRunConfig config) in configuration.SpecialOperationConfig)
                {
                    IReadOnlyList<IOperationProvider> specialOps = Array.Empty<IOperationProvider>();

                    if (config != null)
                    {
                        specialOps = ResolveOperations(config, templateRoot, variables);
                    }

                    RunSpec spec = new RunSpec(specialOps, config?.VariableSetup.FallbackFormat);
