    ...
    public sealed class TemplateConfigModel
    {
        ...
        private static IReadOnlyDictionary<string, IBaselineInfo> BaselineInfoFromJObject(IEnumerable<JProperty> baselineJProperties)
        {
            ...
            foreach (JProperty property in baselineJProperties)
            {
                ...
                var defaultOverrides = obj.Get<JObject>(nameof(IBaselineInfo.DefaultOverrides))?.ToStringDictionary() ?? new Dictionary<string, string>();
                BaselineCacheInfo baseline = new BaselineCacheInfo(defaultOverrides, description: obj.ToString(nameof(IBaselineInfo.Description)));
