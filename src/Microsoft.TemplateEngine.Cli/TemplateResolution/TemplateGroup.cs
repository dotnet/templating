...
#pragma warning disable CS0618 // Type or member is obsolete
                IEnumerable<MatchInfo> singleStartParams = checkTemplate.MatchDisposition.Where(x => x.Kind == MatchKind.SingleStartsWith);
#pragma warning restore CS0618 // Type or member is obsolete
                foreach (var singleStartParam in singleStartParams)
                {
                    if (!singleStartsWithParamNames.Add(singleStartParam.ParameterName))
                    {
                        invalidParameterList.Add(new InvalidParameterInfo(
                                                ...
                                                singleStartParam.ParameterName));
                    }
                }
            }
...
#pragma warning disable CS0618 // Type or member is obsolete
            IEnumerable<MatchInfo> ambiguousParametersForTemplates = Templates.SelectMany(template => template.MatchDisposition.Where(x => x.Kind == MatchKind.AmbiguousParameterValue)).Distinct(new OrdinalIgnoreCaseMatchInfoComparer());
#pragma warning restore CS0618 // Type or member is obsolete
            ...
            IEnumerable<MatchInfo> parametersWithInvalidNames = Templates.SelectMany(template => template.MatchDisposition.Where(x => x.Kind == MatchKind.InvalidParameterName)).Distinct(new OrdinalIgnoreCaseMatchInfoComparer());

            foreach (MatchInfo parameter in parametersWithInvalidNames)
            {
                if (Templates.All(
                    template => template.MatchDisposition.Any(x => x.Kind == MatchKind.InvalidParameterName
                                                                && x.ParameterName.Equals(parameter.ParameterName, StringComparison.OrdinalIgnoreCase))))
                {
                    invalidParameterList.Add(new InvalidParameterInfo(
            ...
            IEnumerable<MatchInfo> invalidParameterValuesForTemplates =
                filteredTemplates
                .SelectMany(template => template.MatchDisposition.Where(x => x.Kind == MatchKind.InvalidParameterValue))
            ...
            foreach (MatchInfo parameter in invalidParameterValuesForTemplates)
            {
                if (filteredTemplates.All(
                   template => template.MatchDisposition.Any(x => x.Kind == MatchKind.InvalidParameterValue
                                                                  && x.ParameterName.Equals(parameter.ParameterName, StringComparison.OrdinalIgnoreCase))))
                {
                    invalidParameterList.Add(new InvalidParameterInfo(
