...
{
    internal class DotnetBuildPostActionProcessor : PostActionProcessor2Base, IPostActionProcessor, IPostActionProcessor2
    {
        private static readonly Guid ActionProcessorId = new Guid("210A437B-A78E-4D2F-B562-4DD3E3EA4025");
        ...
        public bool Process(IEngineEnvironmentSettings environment, IPostAction actionConfig, ICreationEffects2 creationEffects, ICreationResult templateCreationResult, string outputBasePath)
        {
            ...
            foreach (string targetRelativePath in targetFiles)
            {
                ...
                bool succeeded = Callbacks.RestoreProject == null ?
                    Dotnet.Build(pathToRestore).Execute().ExitCode == 0 :
                    Callbacks.RestoreProject(pathToRestore);
