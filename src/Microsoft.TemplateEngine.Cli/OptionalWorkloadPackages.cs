...
{
    internal class OptionalWorkloadPackages : ITemplatesPackagesProviderFactory
    {
        ...
        class OptionalWorkloadProvider : ITemplatesPackagesProvider
        {
            private IEngineEnvironmentSettings EnvironmentSettings;

            public OptionalWorkloadProvider(ITemplatesPackagesProviderFactory factory, IEngineEnvironmentSettings settings)
            {
            ...
            public Task<IReadOnlyList<ITemplatesPackage>> GetAllSourcesAsync(CancellationToken cancellationToken)
            {
                ...
                try
                {
                    var list = new List<TemplatesPackage>();
                    ...
                    {
                        list.Add(new TemplatesPackage(this, packageInfo.Path, fileSystem?.GetLastWriteTimeUtc(packageInfo.Path) ?? File.GetLastWriteTime(packageInfo.Path)));
                    }
