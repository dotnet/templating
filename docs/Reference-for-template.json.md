...
|<a name="defaultValue"></a>`defaultValue`|The value assigned to the symbol if no parameter is provided by the user or host. The default is *not applied* if [`isRequired`](#isRequired) configuration is set to `true` for a parameter (or [`isRequiredCondition`](#isRequiredCondition) is specified and evals to `true`), as that is an indication that user specified value is mandatory.  |
...
|<a name="isRequired"></a>`isRequired`|Indicates if the user supplied value is required or not. If set to `true` and user has not specified the value on input, validation error occurs - even if [`defaultValue`](#defaultValue) is present. <br /> Not allowed together with [`isRequiredCondition`](#isRequiredCondition) property. |
...
|<a name="isEnabledCondition"></a>`isEnabledCondition`| Optional condition indicating whether parameter should be processed. More details in [Conditions documentation](Conditions.md#conditional-parameters). |
|<a name="isRequiredCondition"></a>`isRequiredCondition`| Optional condition indicating whether parameter should be required. More details in [Conditions documentation](Conditions.md#conditional-parameters). <br /> Not allowed together with [`isRequired`](#isRequired) configuration. |
