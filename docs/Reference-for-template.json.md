...
|<a name="defaultValue"></a>`defaultValue`|The value assigned to the symbol if no parameter is provided by the user or host. The default is *not applied* if [`isRequired`](#isRequired) configuration is set to `true` for a parameter (or is set to condition that evals to `true`), as that is an indication that user specified value is mandatory.  |
...
|<a name="isEnabled"></a>`isEnabled`| Optional condition indicating whether parameter should be processed. More details in [Conditions documentation](Conditions.md#conditional-parameters).|
