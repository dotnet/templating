...
namespace Microsoft.TemplateSearch.Common.UnitTests
{
    public class TemplateSearchCacheReaderTests : IClassFixture<EnvironmentSettingsHelper>
    {
        ...
        public void CanReadSearchMetadata()
        {
            ...
            //can read parameters
            Assert.Equal(5, ((ITemplateInfo)parsedCache.TemplatePackages[0].Templates[0]).ParametersDefinition.Count);
        }
        ...
        { 
            ...
            Assert.Equal(2, ((ITemplateInfo)parsedCache.TemplatePackages[0].Templates[0]).ParametersDefinition.Count);

            Assert.Equal(3, ((ITemplateInfo)parsedCache.TemplatePackages[0].Templates[2]).ParametersDefinition.Count);
            Assert.Equal(1, ((ITemplateInfo)parsedCache.TemplatePackages[0].Templates[2]).ParametersDefinition.Where(p => p.DataType == "choice").Count());
            Assert.Equal(3, ((ITemplateInfo)parsedCache.TemplatePackages[0].Templates[2]).ParametersDefinition.Single(p => p.DataType == "choice").Choices?.Count);
        }
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public void CanReadSearchMetadata_V2_E2E()
        {
            ...
            Assert.Equal(3, templateToTest.ParametersDefinition.Count);

            Assert.Single(templateToTest.ParametersDefinition.Where(p => p.DataType == "choice"));
            Assert.Equal(3, templateToTest.ParametersDefinition.Single(p => p.DataType == "choice").Choices?.Count);
            Assert.True(templateToTest.ParametersDefinition.Single(p => p.DataType == "choice").Choices?.ContainsKey("var1"));
